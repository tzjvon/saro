...
exports.getTaroPath = function() {
	const saroPath = path.join(exports.homedir(), '.saro')
	if (!fs.existsSync(saroPath)) {
		fs.mkdirSync(saroPath)
	}
	return saroPath
}

exports.setConfig = function(config) {
	const saroPath = exports.getTaroPath()
	if (typeof config === 'object') {
		const oldConfig = exports.getConfig()
		config = Object.assign({}, oldConfig, config)
		fs.writeFileSync(path.join(saroPath, 'config.json'), JSON.stringify(config, null, 2))
	}
}
